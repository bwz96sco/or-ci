...
Phase 1 supports BWOR-style JSON metadata, Gurobi linear models, ModelIR extraction, and metamorphic verification. The first micro-pilot validates cost scaling; the continuation pilot adds constraint-relaxation checks for selected linear fixtures.

## Terminology

- **OR-CI**: Operations Research Continuous Integration. The package runs CI-style checks for submitted optimization model code.
- **ModelIR**: Optimization Model Intermediate Representation. It is the normalized representation OR-CI extracts from a solver model: variables, bounds, variable types, objective sense and coefficients, constraints, senses, RHS values, and summary counts.
